(function() {
  ...
  var canvas, gl, program, scaleXUniformLocation, scaleX, widen, theta, thetaSpeed, mmLoc, mm, vmLoc, vm, pmLoc, pm, camera, xHurufLocation, triangleX, yHurufLocation, triangleY, zHurufLocation, triangleZ, arahX, arahY, arahZ, dcLoc, dc, ddLoc, dd, acLoc, ac, nmLoc, vNormal, vTexCoord, vColor, flag, flagUniformLocation, fFlagUniformLocation, theta, phi;
  var verticescube = [];
  var cubePoints = [
    [ -0.8, -0.8,  0.8 ],
    [ -0.8,  0.8,  0.8 ],
  ...
  function animasiTranslasi(){
    if (triangleX >= (0.5- Math.abs(0.2 * 0.7 * scaleX))) arahX = -1.0;
    else if (triangleX <= (-0.5 + Math.abs(0.2 * 0.7 * scaleX))) arahX = 1.0;
    triangleX += 0.009 * arahX;
    gl.uniform1f(xHurufLocation, triangleX);
    
    if (triangleY >= (0.7 - (0.3 * 0.7))) arahY = -1.0;
    else if (triangleY <= (-0.7 + (0.3 * 0.7))) arahY = 1.0;
    triangleY += 0.010 * arahY;
    gl.uniform1f(yHurufLocation, triangleY);
    
    if (triangleZ >= (0.75 - Math.abs(0.2 * 0.7 * scaleX))) arahZ = -1.0;
    else if (triangleZ <= (-0.75 + Math.abs(0.2 * 0.7 * scaleX))) arahZ = 1.0;
  ...
  function draw(){
    ...
    ac = glMatrix.vec3.fromValues(0.17, 0.40, 0.76);
